...
Partition &finetune::asynchronous_gibbs(Partition &partition, int num_vertices, int num_edges,
                                        std::vector<Matrix2Column> &out_neighbors,
                                        std::vector<Matrix2Column> &in_neighbors, PartitionTriplet &partitions) {
    if (partition.getNum_blocks() == 1) {
        return partition;
    ...
    for (int iteration = 0; iteration < MAX_NUM_ITERATIONS; ++iteration) {
        ...
        double delta_entropy = 0.0;
        int num_batches = 10;
        int batch_size = int(ceil(num_vertices / num_batches));
        for (int batch = 0; batch < num_vertices / batch_size; ++batch) {
            int start = batch * batch_size;
            int end = std::min(num_vertices, (batch + 1) * batch_size);
            // Block assignment used to re-create the Partition after each batch to improve mixing time of
            // asynchronous Gibbs sampling
            Vector block_assignment(partition.getBlock_assignment());
            #pragma omp parallel for
            for (int vertex = start; vertex < end; ++vertex) {
                VertexMove proposal = propose_gibbs_move(partition, vertex, out_neighbors, in_neighbors);
                if (proposal.did_move) {
                    vertex_moves++;
                    delta_entropy += proposal.delta_entropy;
            ...
            partition = Partition(partition.getNum_blocks(), out_neighbors, partition.getBlock_reduction_rate(),
                                  block_assignment);
        }
        delta_entropies.push_back(delta_entropy);
        std::cout << "Itr: " << iteration << ", number of vertex moves: " << vertex_moves << ", delta S: ";
