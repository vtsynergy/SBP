...
EdgeWeights finetune::block_edge_weights(Vector &block_assignment, EdgeWeights &neighbor_weights) {
...
double finetune::compute_delta_entropy(int current_block, int proposal, Partition &partition, EdgeCountUpdates &updates,
    ...
    Vector new_block_col = common::exclude_indices(updates.block_col, current_block, proposal); // added
    ...
    // Remove 0 indices
    Vector new_block_row_degrees_in = common::index_nonzero(block_degrees.block_degrees_in, updates.block_row); // added
    Vector new_proposal_row_degrees_in = common::index_nonzero(block_degrees.block_degrees_in, updates.proposal_row);
    Vector new_block_row = common::nonzeros(updates.block_row); // added
    Vector new_proposal_row = common::nonzeros(updates.proposal_row);
    Vector new_block_col_degrees_out = common::index_nonzero(new_block_degrees_out, new_block_col); // added
...
bool finetune::early_stop(int iteration, PartitionTriplet &partitions, Partition &partition,
    ...
    if (partitions.get(2).empty) { // Golden ratio bracket not yet established
        threshold = 5e-4 * partition.getOverall_entropy();
    } else {
        threshold = 1e-4 * partition.getOverall_entropy();
    }
...
EdgeCountUpdates finetune::edge_count_updates(BoostMappedMatrix &blockmodel, int current_block, int proposed_block,
    ...
    Vector block_col = blockmodel.getcol(current_block);
...
EdgeWeights finetune::edge_weights(std::vector<Matrix2Column> &neighbors, int vertex) {
    std::vector<int> indices;
    std::vector<int> values;
    Matrix2Column neighbor_matrix = neighbors[vertex];
    for (Eigen::Index row = 0; row < neighbor_matrix.rows(); ++row) {
        indices.push_back(neighbor_matrix(row, 0));
        values.push_back(neighbor_matrix(row, 1));
    }
...
double finetune::hastings_correction(Partition &partition, EdgeWeights &out_blocks, EdgeWeights &in_blocks,
    ...
    Eigen::ArrayXd counts = Eigen::ArrayXd::Zero(num_unique_blocks);
    Eigen::ArrayXd proposal_weights = Eigen::ArrayXd::Zero(num_unique_blocks);
    Eigen::ArrayXd block_weights = Eigen::ArrayXd::Zero(num_unique_blocks);
    Eigen::ArrayXd block_degrees = Eigen::ArrayXd::Zero(num_unique_blocks);
    Eigen::ArrayXd proposal_degrees = Eigen::ArrayXd::Zero(num_unique_blocks);
    // Indexing
    Vector proposal_row = partition.getBlockmodel().getrow(proposal.proposal);
    Vector proposal_col = partition.getBlockmodel().getcol(proposal.proposal);
    // Fill Arrays
    int index = 0;
    int num_blocks = partition.getNum_blocks();
    Vector &current_block_degrees = partition.getBlock_degrees();
    for (auto const &entry : block_counts) {
    ...
    double p_forward = (counts * proposal_weights / block_degrees).sum();
    double p_backward = (counts * block_weights / proposal_degrees).sum();
    if (std::isnan(p_backward / p_forward)) {
        std::cout << "counts: " << counts.transpose() << std::endl;
        std::cout << "proposal weights: " << proposal_weights.transpose() << std::endl;
        std::cout << "block degrees: " << block_degrees.transpose() << std::endl;
        std::cout << "block_weights: " << block_weights.transpose() << std::endl;
        std::cout << "proposal_degrees: " << proposal_degrees.transpose() << std::endl;
        std::cout << "p_f: " << p_forward << " p_b: " << p_backward << std::endl;
        exit(-2);
    }
