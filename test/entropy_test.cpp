...
class UndirectedEntropyTest : public ToyExample {
protected:
    Blockmodel B3;
...
class UndirectedBlockMergeEntropyTest : public BlockMergeTest {
    void SetUp() override {
        args = Args();
        args.undirected = true;
...
TEST_F(EntropyTest, DenseDeltaMDLGivesCorrectAnswer) {
    ...
    double delta_entropy =
            entropy::delta_mdl(current_block, Proposal.proposal, B, Updates, new_block_degrees);
...
TEST_F(EntropyTest, SparseDeltaMDLGivesCorrectAnswer) {
    ...
    double E_before = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    int current_block = B.block_assignment(vertex);
    double delta_entropy = entropy::delta_mdl(current_block, Proposal.proposal, B, SparseUpdates, new_block_degrees);
...
TEST_F(EntropyTest, DeltaMDLUsingBlockmodelDeltasGivesCorrectAnswer) {
    ...
    double E_after = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    EXPECT_FLOAT_EQ(delta_entropy, E_after - E_before)
                        << "calculated dE was " << delta_entropy << " but actual dE was " << E_after << " - "
                        << E_before << " = " << E_after - E_before;
}
...
TEST_F(BlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithDenseUpdates) {
    ...
    double dE = entropy::block_merge_delta_mdl(0, 1, B, Updates, new_block_degrees);
...
TEST_F(BlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithSparseUpdates) {
    double E_before = entropy::mdl(B, 11, 23);
    double dE = entropy::block_merge_delta_mdl(0, 1, B, SparseUpdates, new_block_degrees);
    double E_after = entropy::mdl(B2, 11, 23);
    EXPECT_FLOAT_EQ(E_after - E_before, dE);
}
...
TEST_F(UndirectedEntropyTest, MDLGivesCorrectAnswer) {
    double E = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    EXPECT_FLOAT_EQ(E, ENTROPY) << "Calculated entropy = " << E << " but was expecting " << ENTROPY;
}

/// TODO: same test but using a vertex with a self edge
TEST_F(UndirectedEntropyTest, DenseDeltaMDLGivesCorrectAnswer) {
    int vertex = 7;
    double E_before = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    int current_block = B.block_assignment(vertex);
    double delta_entropy =
...
TEST_F(UndirectedEntropyTest, SparseDeltaMDLGivesCorrectAnswer) {
    int vertex = 7;
    double E_before = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    int current_block = B.block_assignment(vertex);
    double delta_entropy = entropy::delta_mdl(current_block, Proposal.proposal, B, SparseUpdates, new_block_degrees);
...
/// TODO: same test but using a vertex with a self edge
TEST_F(UndirectedEntropyTest, DeltaMDLUsingBlockmodelDeltasGivesCorrectAnswer) {
    int vertex = 7;
    double E_before = entropy::mdl(B, graph.num_vertices(), graph.num_edges());
    double delta_entropy = entropy::delta_mdl(B, Deltas, Proposal);
    B.move_vertex(vertex, Deltas, Proposal);
...
TEST_F(UndirectedEntropyTest, HastingsCorrectionBlockCountsAreTheSameWithAndWithoutBlockmodelDeltas) {
    int vertex = 7;
    MapVector<int> block_counts1;
//    std::unordered_map<int, int> block_counts1;
    for (const int neighbor: graph.out_neighbors(vertex)) {
...
TEST_F(UndirectedEntropyTest, HastingsCorrectionWithAndWithoutDeltaGivesSameResult) {
    int vertex = 7;
    int current_block = B.block_assignment(vertex);
    double hastings1 = entropy::hastings_correction(vertex, graph, B, Deltas, current_block, Proposal);
    EdgeWeights out_edges = finetune::edge_weights(graph.out_neighbors(), vertex);
...
TEST_F(UndirectedEntropyTest, SpecialCaseShouldGiveCorrectDeltaMDL) {
    int vertex = 6;
    utils::ProposalAndEdgeCounts proposal{0, 1, 2, 3};
    EdgeWeights out_edges = finetune::edge_weights(graph.out_neighbors(), vertex, false);
    EdgeWeights in_edges = finetune::edge_weights(graph.in_neighbors(), vertex, true);
...
TEST_F(UndirectedEntropyTest, NullModelMDLv1ShouldGiveCorrectMDLForSmallGraph) {
    double mdl = entropy::null_mdl_v1(50);
    EXPECT_FLOAT_EQ(mdl, MDL_10_VERTICES_50_EDGES_V1);
}

TEST_F(UndirectedEntropyTest, NullModelMDLv2ShouldGiveCorrectMDLForSmallGraph) {
    double mdl = entropy::null_mdl_v2(10, 50);
    EXPECT_FLOAT_EQ(mdl, MDL_10_VERTICES_50_EDGES_V2);
}

TEST_F(UndirectedEntropyTest, NullModelMDLv1ShouldGiveCorrectMDLForLargeGraph) {
    double hand_calculated_mdl = 1.462023E14;
    long E = 5E12;
    long V = 1E12;
    double blocks = 1.0;
...
TEST_F(UndirectedEntropyTest, NullModelMDLv2ShouldGiveCorrectMDLForLargeGraph) {
    double hand_calculated_mdl = 3.089407E+14;
    long E = 5E12;
    long V = 1E12;
    auto blocks = double(V);
...
TEST_F(UndirectedBlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithDenseUpdates) {
    double E_before = entropy::mdl(B, 11, 23);
    double dE = entropy::block_merge_delta_mdl(0, 1, B, Updates, new_block_degrees);
    double E_after = entropy::mdl(B2, 11, 23);
    EXPECT_FLOAT_EQ(E_after - E_before, dE);
...
TEST_F(UndirectedBlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithSparseUpdates) {
    double E_before = entropy::mdl(B, 11, 23);
    double dE = entropy::block_merge_delta_mdl(0, 1, B, SparseUpdates, new_block_degrees);
    double E_after = entropy::mdl(B2, 11, 23);
    EXPECT_FLOAT_EQ(E_after - E_before, dE);
...
TEST_F(UndirectedBlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithBlockmodelDeltas) {
    double E_before = entropy::mdl(B, 11, 23);
    double dE = entropy::block_merge_delta_mdl(0, B, Deltas, new_block_degrees);
    double E_after = entropy::mdl(B2, 11, 23);
    EXPECT_FLOAT_EQ(E_after - E_before, dE);
...
TEST_F(UndirectedBlockMergeEntropyTest, BlockmodelDeltaMDLIsCorrectlyComputeWithBlockmodelDeltasSansBlockDegrees) {
    double E_before = entropy::mdl(B, 11, 23);
    double dE = entropy::block_merge_delta_mdl(0, {1, B.degrees_out(0),
                                               B.degrees_in(0), B.degrees(0)}, B, Deltas);
    double E_after = entropy::mdl(B2, 11, 23);
