...
class TwoHopBlockmodel : public Blockmodel {
  public:
    ...
    TwoHopBlockmodel(int num_blocks, const Graph &graph, float block_reduction_rate)
        ...
        this->initialize_edge_counts(graph.out_neighbors());
    }
    TwoHopBlockmodel(int num_blocks, const Graph &graph, float block_reduction_rate,
  ...
  private:
    ...
    /// 2-Hop data distribution, based on snowball sampling and super-vertices (supernodes), each MPI rank responsible
    /// for the vertices in the blocks mapped to it.
    void distribute_2hop_super_snowball(const NeighborList &out_neighbors, const NeighborList &in_neighbors);
    // ===== Variables
