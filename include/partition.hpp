...
Graph* partition(Graph* graph, long rank, long num_processes, Args &args);

/// Partitions the graph using the round robin strategy. The resulting partitions do not overlap.
Graph* partition_round_robin(Graph* graph, long rank, long num_processes, long target_num_vertices);

/// Partitions the graph using the random strategy. The resulting partitions do not overlap.
Graph* partition_random(Graph* graph, long rank, long num_processes, long target_num_vertices);

/// Partitions the graph using the snowball sampling strategy. The resulting partitions DO overlap.
Graph* partition_snowball(Graph* graph, long rank, long num_processes, long target_num_vertices);
